...
The itch.io app supports two sandbox backends on Linux.
...
2. **Firejail** — uses a system install if available, otherwise the app installs its own copy
...
- System directories (`/usr`, `/lib`, `/lib64`, `/bin`, `/sbin`, `/etc`, `/sys`) are mounted **read-only**
...
[Firejail](https://github.com/netblue30/firejail) is an alternative sandbox that uses a blacklist-based filesystem policy. It generates a per-game profile at `{install_folder}/.itch/isolate-app.profile`.

When Auto falls back to Firejail, the app first looks for a system-installed `firejail` in your `PATH`. If not found, the app downloads and installs its own copy.

Firejail is generally **not recommended**. Unlike Bubblewrap, which starts from a minimal environment and explicitly mounts only what's needed, Firejail allows access to most of the filesystem and selectively blocks known sensitive paths. This means its sandbox coverage is less comprehensive.
...
- **Sandbox type** — Choose between **Auto**, **Bubblewrap**, or **Firejail**
...
The sandbox profile is designed for **compatibility**: it shares the X11 display sockets, session D-Bus, and input devices (`/dev/input`) with the host so that games work out of the box.
...
Wayland isolates clients at the display protocol level, but access to shared resources such as `/dev/input` and the session D-Bus remains.
